...
export const TaskCard = React.memo(
  ({ task, onUpdate, onDelete, onDuplicate, onEdit }: TaskCardProps) => {
    const handleDelete = () => {
      onDelete(task.id);
    };
      ...
      >
        ...
        <div className="flex flex-wrap gap-1.5 mt-auto border-t border-gray-200 pt-3">
          ...
          {onEdit && (
            <button
              onClick={handleEdit}
              className="flex-1 min-w-[100px] flex items-center justify-center gap-1.5 bg-green-500 text-white py-2 px-3 rounded-md text-xs font-medium hover:bg-green-600 transition-colors relative"
              title="Edit this task"
              disabled={isPending}
            >
              <PencilIcon className="shrink-0" />
              <span>Edit</span>
              {isPending && <Loading className="absolute right-2" />}
            </button>
          )}
          <button
